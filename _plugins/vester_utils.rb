	...
	module VesterUtils
  	extend self
		...
		def create_geneology_hash(page, pages)	
			...
			hash['ancestors'].pop if page['url'] == '/'	
			hash['root'] = hash['ancestors'].last || '/'
			hash['ancestors'] << '//vesterheim.org/'
...
#STDERR.puts root_nodes.inspect
...
#STDERR.puts page['metadata'].inspect
			navigation = page['navigation'] || {}
			isnt_navigable = 
